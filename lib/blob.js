...
export async function getData() {
  ...
  if (useBlob) {
    for (let attempt = 0; attempt < 5; attempt += 1) {
      try {
        ...
        const res = await fetch(`${blob.url}?v=${Date.now()}-${attempt}`, {
          cache: "no-store",
        });
        if (res.ok) {
          latestData = normalizeData(await res.json());
          return latestData;
        }
      } catch (err) {
        if (attempt === 4) console.error("Gagal membaca data dari Vercel Blob:", err);
      }
  ...
  try {
    ...
    latestData = normalizeData(JSON.parse(raw));
    return latestData;
  } catch {
    return seedData();
  }
...
export async function saveData(data) {
  ...
  fs.writeFileSync(LOCAL_DATA_PATH, JSON.stringify(latestData, null, 2));
}
